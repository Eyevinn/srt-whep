...
| Half-attached branch on `add_branch` failure | **Detach it before replying with the error.** The id was never inserted into the map, so DELETE could never reach it; `detach` tolerates a half-built branch. As of the C1+C2a refactor (2026-07-09) this detach lives inside `add_branch` itself rather than in the coordinator; the semantic — detached before the error reply — is unchanged. |
